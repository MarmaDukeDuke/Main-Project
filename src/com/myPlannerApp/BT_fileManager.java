...
public class BT_fileManager {
	...
	public static boolean doesProjectAssetExist(String assetsDirectory, String fileName){
        boolean ret = false;
        final AssetManager am = myPlannerApp_appDelegate.getApplication().getAssets();
        try{
	...
	public static boolean copyAssetToCache(String assetsDirectory, String fileName){
		...
		try{
		    ...
		    FileOutputStream outputStream = myPlannerApp_appDelegate.getApplication().openFileOutput(fileName, Context.MODE_WORLD_READABLE);
		    InputStream inputStream = myPlannerApp_appDelegate.getApplication().getAssets().open(assetsDirectory + "/" + fileName);
	...
	public static boolean copyFileFromCacheToSDCard(String fileName){
		...
		try{
		   	...
		   	//copy from assets to internal cache...
			FileInputStream fin = myPlannerApp_appDelegate.getApplication().openFileInput(fileName);
            File file = new File(myPlannerApp_appDelegate.getApplication().getExternalCacheDir(), fileName);
	...
	public static boolean doesCachedFileExist(String fileName){
		...
		try{
			...
			File file = myPlannerApp_appDelegate.getApplication().getFileStreamPath(fileName);
			if(file.exists()){
				ret = true;
			}
		}catch(Exception e){
	...
	public static int getResourceIdFromBundle(String resourcesDirectory, String fileName){
		...
		try{
			...
			ret = myPlannerApp_appDelegate.getApplication().getResources().getIdentifier(useResName, resourcesDirectory, myPlannerApp_appDelegate.getApplication().getPackageName());
	...
	public static Drawable getDrawableByName(String fileName){
		...
		try{
			...
			//make sure filename is a string...
			useResName = "" + useResName;
			int resId = myPlannerApp_appDelegate.getApplication().getResources().getIdentifier(useResName, "drawable", myPlannerApp_appDelegate.getApplication().getPackageName());
			d = myPlannerApp_appDelegate.getApplication().getResources().getDrawable(resId);
	...
	public static void saveImageToCache(Bitmap theImage, String saveAsFileName){
		...
		try{
			if(saveAsFileName.length() > 3 && theImage != null){
				FileOutputStream fos = myPlannerApp_appDelegate.getApplication().openFileOutput(saveAsFileName, Context.MODE_WORLD_READABLE);
	...
	public static void saveTextFileToCache(String theData, String saveAsFileName){
		...
		try{
			if(saveAsFileName.length() > 3){
				OutputStreamWriter out = new OutputStreamWriter(myPlannerApp_appDelegate.getApplication().openFileOutput(saveAsFileName, Context.MODE_WORLD_READABLE));
				out.write(theData);
				out.close();
			}
		}catch (Exception e) {
	...
	public static String readTextFileFromAssets(String assetsDirectory, String fileName){
		...
		String ret = "";
		final AssetManager am = myPlannerApp_appDelegate.getApplication().getResources().getAssets();
		InputStream is = null;
		try{
	...
	public static int getCachedDataSize(){
		...
		int ret = 0;
		try{
			String appDataDirectory = myPlannerApp_appDelegate.getApplication().getFilesDir().getAbsolutePath();

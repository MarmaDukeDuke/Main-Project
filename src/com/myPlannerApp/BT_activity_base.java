...
public class BT_activity_base extends Activity implements LocationListener{
    ...
    public void onCreate(Bundle savedInstanceState){
		...
		if(myPlannerApp_appDelegate.rootApp.getPromptForPushNotifications().equals("1")  && myPlannerApp_appDelegate.rootApp.getRegisterForPushURL().length() > 3){
			promptForPush = true;
		}
		...
		if(promptForPush){
			try{
		        ...
		        registerReceiver(baseHandlePushReceiver, new IntentFilter("com.myPlannerApp.DISPLAY_MESSAGE"));        	
		...
		if(tabIndex > -1){
        	...
        	if(tabIndex == 0) this.screenData = myPlannerApp_appDelegate.rootApp.getTab0ScreenData();
        	if(tabIndex == 1) this.screenData = myPlannerApp_appDelegate.rootApp.getTab1ScreenData();
        	if(tabIndex == 2) this.screenData = myPlannerApp_appDelegate.rootApp.getTab2ScreenData();
        	if(tabIndex == 3) this.screenData = myPlannerApp_appDelegate.rootApp.getTab3ScreenData();
        	if(tabIndex == 4) this.screenData = myPlannerApp_appDelegate.rootApp.getTab4ScreenData();
		
		}else{
			
			//set the screen data..
			this.screenData = myPlannerApp_appDelegate.rootApp.getCurrentScreenData();
        ...
        if(!BT_fileManager.doesCachedFileExist(myPlannerApp_appDelegate.cachedConfigModifiedFileName)){
        	BT_fileManager.saveTextFileToCache("{this is a pretend date}", myPlannerApp_appDelegate.cachedConfigModifiedFileName);
        }
	...
	protected void onStart() {
		...
		super.onStart();
		if(!myPlannerApp_appDelegate.foundUpdatedLocation && myPlannerApp_appDelegate.rootApp.getRootDevice().canUseGPS()){
			if(myPlannerApp_appDelegate.rootApp.getStartLocationUpdates().equalsIgnoreCase("1")){
    ...
    public void onResume() {
       ...
       //BT_debugger.showIt(activityName + ":onResume");
		...
		if(this.screenData.isHomeScreen()){
			...
			//must have dataURL's
			if(myPlannerApp_appDelegate.rootApp.getDataURL().length() > 1 && myPlannerApp_appDelegate.rootApp.getReportToCloudURL().length() > 1){
		...
		super.onConfigurationChanged(newConfig);
		  switch(newConfig.orientation){
	      		case  Configuration.ORIENTATION_LANDSCAPE:
	      			...
	      			myPlannerApp_appDelegate.rootApp.getRootDevice().updateDeviceOrientation("landscape");
				break;
	      		case Configuration.ORIENTATION_PORTRAIT:
	    			BT_debugger.showIt(activityName + ":onConfigurationChanged to portrait");
	      			myPlannerApp_appDelegate.rootApp.getRootDevice().updateDeviceOrientation("portrait");
	    			break;
	...
	public void registerForPush(){
		...
		if(!gcmRegId.equals("") && doSetupForPush) {
            ...
            if(GCMRegistrar.isRegisteredOnServer(this)) {
            	...
            	myPlannerApp_appDelegate.rootApp.setPushRegistrationId(gcmRegId);
	...
	private Handler mHandler = new Handler(){
		...
		public void handleMessage(Message msg){
			...
			String cachedConfigModifiedFileName = myPlannerApp_appDelegate.cachedConfigModifiedFileName;
	...
	public void reportToCloud(){
		...
		new Thread(){
			...
			public void run(){
				...
				String dataURL = BT_strings.mergeBTVariablesInString(myPlannerApp_appDelegate.rootApp.getDataURL());
				String reportToCloudURL = BT_strings.mergeBTVariablesInString(myPlannerApp_appDelegate.rootApp.getReportToCloudURL());
				...
				if(dataURL.length() > 5 && reportToCloudURL.length() > 5){
					...
					if(myPlannerApp_appDelegate.rootApp.getCurrentMode().length() > 1){
						reportToCloudURL += "&currentMode=" + myPlannerApp_appDelegate.rootApp.getCurrentMode();
					}					
	...
	////////////////////////////////////////////////////////////////////////////////////////////////////////////
     ...
     public void getLastLocation(){
         try{
 			...
 			if(!myPlannerApp_appDelegate.foundUpdatedLocation){
 	        	...
 	        	if(this.locationManager != null){
 	            	...
 	            	if(lastLocation != null){
 	            		...
 	            		myPlannerApp_appDelegate.rootApp.getRootDevice().setDeviceLatitude(String.valueOf(lastLocation.getLatitude()));
 	            		myPlannerApp_appDelegate.rootApp.getRootDevice().setDeviceLongitude(String.valueOf(lastLocation.getLongitude()));
 	...
 	public void onLocationChanged(Location location){
 		this.locationUpdateCount++;
  		...
  		try{
	 	    ...
	 	    myPlannerApp_appDelegate.rootApp.getRootDevice().setDeviceLatitude(String.valueOf(location.getLatitude()));
	 	    myPlannerApp_appDelegate.rootApp.getRootDevice().setDeviceLongitude(String.valueOf(location.getLongitude()));
	 		...
	 		if(locationUpdateCount > 10 || location.getAccuracy() < 25){
	 	  		...
	 	  		myPlannerApp_appDelegate.foundUpdatedLocation = true;

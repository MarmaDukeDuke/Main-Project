...
public class BT_screen_menuButtons extends BT_activity_base implements OnClickListener{
    ...
    public void onClick(View view){
    	...
    	if(loadScreenWithItemId.length() > 1 && !loadScreenWithItemId.equalsIgnoreCase("none")){
    		...
    		tapScreenLoadObject = myPlannerApp_appDelegate.rootApp.getScreenDataByItemId(loadScreenWithItemId);
    	}else{
    		if(loadScreenWithNickname.length() > 1){
    			...
    			tapScreenLoadObject = myPlannerApp_appDelegate.rootApp.getScreenDataByItemNickname(loadScreenWithNickname);
    		}else{
    			try{
    ...
    public void layoutButtons(){
        ...
        //device size...
        int deviceWidth = myPlannerApp_appDelegate.rootApp.getRootDevice().getDeviceWidth();
        @SuppressWarnings("unused")
		int deviceHeight = myPlannerApp_appDelegate.rootApp.getRootDevice().getDeviceHeight();
                
        //if we are in landscape orientation the sizes are backwards (intentionally, see BT_device class)...
		if(myPlannerApp_appDelegate.rootApp.getRootDevice().getDeviceOrientation().equalsIgnoreCase("landscape")){
			deviceWidth = myPlannerApp_appDelegate.rootApp.getRootDevice().getDeviceHeight();
	        deviceHeight = myPlannerApp_appDelegate.rootApp.getRootDevice().getDeviceWidth();
		}
	        ...
	        while (cnt < numButtons) {
	            ...
	            for(int c = 0; c < cols; c++){
	        		...
	        		if(cnt < numButtons){
		                ...
		                tr.addView(buttonBox, buttonSize + buttonPadding, buttonSize + buttonPadding + addHeight);
        		...
        		if(addHeight > 0){
        			...
        			spacerRow.addView(spacerBox, 15, 15);

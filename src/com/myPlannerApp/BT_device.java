...
public class BT_device {
	...
	public BT_device(){
		...
		try{
			...
			//app delegate
			Application appDelegate = myPlannerApp_appDelegate.getApplication();
			...
			PackageManager pm = myPlannerApp_appDelegate.getApplication().getPackageManager();
			...
			//is GPS capable..
     		try{
     	        myPlannerApp_appDelegate.getApplication();
				LocationManager lm = (LocationManager) myPlannerApp_appDelegate.getApplication().getSystemService(Context.LOCATION_SERVICE);  
     	        if(lm != null){
 					this.canUseGPS = true;

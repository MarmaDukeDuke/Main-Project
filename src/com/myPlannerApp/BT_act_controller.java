...
public class BT_act_controller {
	...
	public static void handleRightNavButton(final Activity parentActivity, final BT_item parentActivityScreenData, final BT_item theScreenData){
    	...
    	if(loadScreenItemId.length() > 1 && !loadScreenItemId.equalsIgnoreCase("none")){
    		...
    		tapScreenLoadObject = myPlannerApp_appDelegate.rootApp.getScreenDataByItemId(loadScreenItemId);
    	}else{
    		if(loadScreenNickname.length() > 1){
    			...
    			tapScreenLoadObject = myPlannerApp_appDelegate.rootApp.getScreenDataByItemNickname(loadScreenNickname);
    		}else{
    			try{
    	...
    	if(tapScreenLoadObject != null){
    		...
    		BT_activity_base.showAlertFromClass(myPlannerApp_appDelegate.getApplication().getString(R.string.errorTitle), myPlannerApp_appDelegate.getApplication().getString(R.string.errorNoScreenConnected));
    	}
	...
	public static void loadScreenObject(Activity parentActivity, BT_item parentScreenData, BT_item theMenuItemData, BT_item theScreenData){
        ...
        if(BT_strings.getJsonPropertyValue(theScreenData.getJsonObject(), "loginRequired", "").equalsIgnoreCase("1")){
        	if(!myPlannerApp_appDelegate.rootApp.getRootUser().getIsLoggedIn()){
        ...
        if(!allowNextScreen){
    		BT_activity_base.showAlertFromClass(myPlannerApp_appDelegate.getApplication().getString(R.string.logInRequiredTitle), myPlannerApp_appDelegate.getApplication().getString(R.string.logInRequiredMessage));
    		//bail
    		return;
        }
        ...
        if(allowNextScreen){
        	...
        	if(theMenuItemData != null){
	            ...
	            if(BT_strings.getJsonPropertyValue(theMenuItemData.getJsonObject(), "soundEffectFileName", "").length() > 1){
	            	myPlannerApp_appDelegate.playSoundEffect(BT_strings.getJsonPropertyValue(theMenuItemData.getJsonObject(), "soundEffectFileName", ""));
	            }
	            ...
	            myPlannerApp_appDelegate.rootApp.setPreviousMenuItemData(myPlannerApp_appDelegate.rootApp.getCurrentMenuItemData());
	    		            
	    		//remember current menu object
	            myPlannerApp_appDelegate.rootApp.setCurrentMenuItemData(theMenuItemData);
	         
        	}
        	
    		//remember current screen object
        	myPlannerApp_appDelegate.rootApp.setCurrentScreenData(theScreenData);
            
    		//remember previous screen object
        	myPlannerApp_appDelegate.rootApp.setPreviousScreenData(parentScreenData);
            	...
            	if(appToLaunch.length() > 1 && dataURL.length() > 1){
        			...
        			try{
        				...
        				if(appToLaunch.equalsIgnoreCase("browser")){
        			    	  ...
        			    	  parentActivity.startActivity(Intent.createChooser(intent, myPlannerApp_appDelegate.getApplication().getString(R.string.openWithWhatApp)));  
         				}
            ...
            myPlannerApp_appDelegate.rootApp.getTransitionTypeHistory().add(theTransitionType);
			
			//create the new intent
			try{
				
				//create an intent using theScreenData's "class name"
                String theActivityClass = "com.myPlannerApp." + nextScreenType;
				...
				BT_activity_base.showAlertFromClass(myPlannerApp_appDelegate.getApplication().getString(R.string.errorTitle), myPlannerApp_appDelegate.getApplication().getString(R.string.errorLoadingScreen));
	...
	public static void sendEmailWithScreenData(final Activity parentActivity, final BT_item theScreenData){
		...
		if(myPlannerApp_appDelegate.rootApp.getRootDevice().canSendEmail()){
			...
			try{
	    	  ...
	    	  parentActivity.startActivity(Intent.createChooser(intent, myPlannerApp_appDelegate.getApplication().getString(R.string.openWithWhatApp)));  
	    		...
	    		BT_activity_base.showAlertFromClass(myPlannerApp_appDelegate.getApplication().getString(R.string.noNativeAppTitle), myPlannerApp_appDelegate.getApplication().getString(R.string.noNativeAppDescription));
	    	}
	    	
		}else{
			BT_activity_base.showAlertFromClass(myPlannerApp_appDelegate.getApplication().getString(R.string.errorTitle), myPlannerApp_appDelegate.getApplication().getString(R.string.noNativeAppTitle));
		}
	}
	...
	public static void sendSMSWithScreenData(final Activity parentActivity, final BT_item theScreenData){
		...
		if(myPlannerApp_appDelegate.rootApp.getRootDevice().canSendSMS()){
	...
	public static void launchPhoneDialerWithScreenData(final Activity parentActivity, final BT_item theScreenData){
		...
		if(myPlannerApp_appDelegate.rootApp.getRootDevice().canMakeCalls()){
